...
func (r *gormRepository) GetManagerByUserID(ctx context.Context, userID uint) (*domain.Manager, error) {
	...
	return &domain.Manager{
		...
		User: authDomain.User{
			...
			HashedPassword: model.User.HashedPassword,
			ProfilePic:     model.User.ProfilePic,
			City:           model.User.City,
			Country:        model.User.Country,
			DateOfBirth:    model.User.DateOfBirth,
			LastLoginAt:    model.User.LastLoginAt,
		},
		OrganisationID: model.OrganisationID,
		Organisation: orgDomain.Entity{
			ID:                 model.Organisation.ID,
			CompanyName:        model.Organisation.CompanyName,
			CompanyCode:        model.Organisation.CompanyCode,
			PrimaryContactName: model.Organisation.PrimaryContactName,
			PrimaryEmail:       model.Organisation.PrimaryEmail,
			PhoneNumber:        model.Organisation.PhoneNumber,
			OfficeAddress:      model.Organisation.OfficeAddress,
			City:               model.Organisation.City,
			State:              model.Organisation.State,
			PostalCode:         model.Organisation.PostalCode,
			Country:            model.Organisation.Country,
			PlanID:             model.Organisation.PlanID,
			PlanStartDate:      model.Organisation.PlanStartDate,
			PlanEndDate:        model.Organisation.PlanEndDate,
			PlanStatus:         model.Organisation.PlanStatus,
			MaxEmployees:       model.Organisation.MaxEmployees,
			CompanyLogo:        model.Organisation.CompanyLogo,
			Industry:           model.Organisation.Industry,
			BillingAddress:     model.Organisation.BillingAddress,
			CompanySize:        model.Organisation.CompanySize,
			BossID:             model.Organisation.BossID,
		},
		Email:       model.Email,
		Type:        model.Type,
		Salary:      model.Salary,
...
func toDomain(m *models.Employee) *domain.Employee {
	return &domain.Employee{
		...
		Organisation: orgDomain.Entity{
			...
			CompanyCode:        m.Organisation.CompanyCode,
			PrimaryContactName: m.Organisation.PrimaryContactName,
			PrimaryEmail:       m.Organisation.PrimaryEmail,
			PhoneNumber:        m.Organisation.PhoneNumber,
			OfficeAddress:      m.Organisation.OfficeAddress,
			City:               m.Organisation.City,
			State:              m.Organisation.State,
			PostalCode:         m.Organisation.PostalCode,
			Country:            m.Organisation.Country,
			PlanID:             m.Organisation.PlanID,
			PlanStartDate:      m.Organisation.PlanStartDate,
			PlanEndDate:        m.Organisation.PlanEndDate,
			PlanStatus:         m.Organisation.PlanStatus,
			MaxEmployees:       m.Organisation.MaxEmployees,
			CompanyLogo:        m.Organisation.CompanyLogo,
			Industry:           m.Organisation.Industry,
			BillingAddress:     m.Organisation.BillingAddress,
			CompanySize:        m.Organisation.CompanySize,
			BossID:             m.Organisation.BossID,
		},
		Email:       m.Email,
		Type:        m.Type,
		Salary:      m.Salary,
